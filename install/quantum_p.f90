...
! Run type -- shape of jellium (sphere = 1, diode = 2)
  integer           :: run_type
...
!    write(6,*) V_tot1,V_tot2
  ...
  write(6,*) sum(rho_bg)*grid_volume
  density=rho_bg
  call solve_poisson
  V_ext=-VH
...
FUNCTION check_pos(r,R0,Ltot,theta)
  !check if a point r is within a sharp tipped cylinder
...
!!! small corrections still needed. See notes
  ...
  allocate(tv(N_L_points),tv0(N_L_points))
  do i=1,N_L_points
    t=h2m*((g_vectors(1,i)+A_vec(1)/(hbar))**2+(g_vectors(2,i)+A_vec(2)/(hbar))**2+(g_vectors(3,i)+A_vec(3)/(hbar))**2)
    tv(i)=exp(-0.5d0*zi*time_step*t/hbar)
    t=h2m*((g_vectors(1,i))**2+(g_vectors(2,i))**2+(g_vectors(3,i))**2)
    tv0(i)=exp(0.5d0*zi*time_step*t/hbar)
  end do
  do j=1,N_orbitals
    Phi_g(:)=Psi_so(:,j)
...
!  call save_1D_proj_real(J_z,'flux',"./flux/",0,0.d0)
  ...
  do it=0,N_time
    ...
    if(.not.dielectric) then
      if(t<3.d0*tt) then
        E_field=E_0*sin(pi*t/(6.d0*tt))*sin(omega*t)
      else
        E_field=E_0*sin(omega*t)
      endif
